...
| **Events from a calendar** | Upcoming events from any calendar entity, one per card. Titles are cleaned for the board: accents dropped, smart quotes and dashes flattened, emoji removed — `TOMORROW 3 PM` on the first row, the event below it, `NOW` while it is on. Choose how many days ahead and how many events at most. A card leaves the board when its event ends |
